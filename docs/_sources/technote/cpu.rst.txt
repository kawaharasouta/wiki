...
チャタリングとかCRフィルタとかシュミットトリガとかの説明もしかしたらした方がいいのかもしれないけど
とりあえず今はかかなくていいかな．

::

  今チャタリング: スイッチの接点の衝突により発生する物理的なバウンド．
  今CRフィルタ: コンデンサへの電荷の貯まりに時間をかけることによってチャタリングを防止する．
  今シュミットトリガ: 回路にHでもLでもない不安定な状態の電圧が入力されることがあるとき，それを綺麗に整形するみたいなやつ．


リセット回路
=============

CPUが動くときに一番最初に必要になるやつ．．．みたいな．こんな書き方でいいのか．
もちろん設計によるけど，スイッチ押したらL(またはH)になるみたいな回路作ればいい．
クロックジェネレータ見ればわかるから説明省く．
ちなみに，リセット回路のスイッチを手動でぽちぽちすれば手動クロックになるぜ．
...
.. image:: ./clock_generator_H.png
   :scale:  40%
   :align: center

ちなみにこのときAの電圧は プルアップ電圧+しきい値
こうなると今度はコンデンサが放電，電荷がなくなると以前とは逆向きの電荷を貯め初めて，またしきい値まで電荷がたまると点AがLになる．
これが電源が供給されている間ずっと繰り返される．

※右端が出力となるような形のクロックジェネレータの回路を一応載せておく※

.. image:: ./clock_generator_2.png
   :scale:  40%
   :align: center
